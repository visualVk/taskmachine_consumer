...
import okhttp3.*;
import org.wwx.client.constant.TaskUrl;
...
// 利用OKhttp发起端口请求
public class TaskMachineServerImpl implements TaskMachineServer {
    ...
    // 调用创建任务接口
    @Override
    public ReturnStatus createTask(TaskMachineTaskRequest taskMachineTaskRequest) {
        return post(TaskUrl.CREATE_TASK, taskMachineTaskRequest);
    }

    // 调用更改任务信息接口
    @Override
    public ReturnStatus setTask(TaskMachineTaskSetRequest taskMachineTaskSetRequest) {
        return post(TaskUrl.SET_TASK, taskMachineTaskSetRequest);
    }
